...
Each batch on `/admin/batches` has an **Edit** disclosure to change its teacher, day/time, or room after creation (`updateBatch()`) — the same day/time picker as "New batch", pre-filled with its current values. Instrument isn't editable on the *batch* itself — a batch's `Course` is fixed at creation, so switching one to a different instrument goes through delete + create a fresh batch instead. (A *student's* enrollment is different — see "Moving a student to a different instrument" under Class packages & renewals.)

An `INACTIVE` student never shows up in a batch's roster, its student count, or the day's attendance-marking list — even if their individual `Enrollment` rows are still `ACTIVE` (marking a student inactive doesn't touch those, so re-activating them doesn't require re-enrolling anywhere). Every one of these views filters on the student's own status, not just the enrollment's, so this stays true everywhere a roster is built from `Enrollment`. That includes the "Other students (reschedule / comp)" list on `/admin/attendance` and `/teacher/attendance` — built from that day's `Attendance` rows rather than `Enrollment`, so it needed the same `student.status` filter separately — and the dashboard's "Classes today" roster-size/marked counts. Deleting a batch follows the same rule: only an enrolled *active* student blocks it (with a clear error naming how many), while any leftover enrollment from an inactive student is cleaned up automatically rather than blocking a deletion that looks — and is — perfectly safe.

## Dashboard

`/admin` (and `/admin` alone) is the daily operating snapshot, not just a link hub:

- **Stat row 1**: Open leads (with a New/Contacted/Trial breakdown as the hint), Active students, **Renewals due soon**, Classes today — each links straight into the matching filtered view (see "Finding who needs a renewal" and the batch-filter sections above). "Renewals due soon" counts active subscriptions with fewer than 6 classes remaining — computed live from attendance, exactly like the students list — regardless of calendar expiry date; a student burning through classes fast surfaces here even with weeks left on the clock. (Distinct from the per-subscription "Renew soon" badge elsewhere, which is a tighter ≤ 2 threshold.)
...
- **Expiry & attendance watch**: two lists worth a glance every month — **Expiring this month** (the calendar-expiry signal the "Renewals due soon" stat used to carry, before that became purely about classes remaining), and **Irregular attendance**: an active subscription with classes still remaining but no `PRESENT` record in the last 20 days — that student is paying for classes they aren't using. A subscription younger than 20 days is never flagged irregular; it hasn't had a fair chance to go quiet yet. Both lists cap at 6 rows with a "+N more" count rather than growing unbounded.

## How the workflow fits together

1. **Leads** (`/admin/leads`) — log an enquiry from a call or walk-in, track follow-ups, update status. Searchable by name/phone/email. "Edit details" lets you correct any captured field later — if the lead has already been converted, its name/phone/email are kept in sync onto the linked student too, since a typo caught after conversion (or the name being captured wrong at enquiry time) shouldn't need fixing twice in two different places.
...
- **Logging a past subscription cycle after the fact**: "+ Add a past subscription" (next to the "past subscriptions" list) creates a subscription row directly with a closed status (`EXPIRED` or `CANCELLED`, your choice) and its own plan, dates, and `classesUsedAtClose` — a distinct historical record, not folded into the current subscription's offset. Available regardless of whether the student has a current active subscription, since past history is independent of it; never touches the current one.
- **Fixing or removing a past subscription**: each entry in the "past subscriptions" list has its own "Edit" (same fields as adding one, pre-filled — plan, dates, classes completed, how it ended) and "Delete" (with a confirmation prompt, since it's not reversible). Both refuse to touch the active subscription — that has its own "Edit subscription" / "Cancel subscription" instead, since deleting an active subscription outright (versus cancelling it) would lose its usage history.
...
- **Finding who needs a renewal** (`/admin/students`): every row shows each active subscription's used/total classes and expiry date directly, colored by urgency. Four combinable filters — status, instrument, renewal state (renewing this month / fewer than 6 left / renew soon ≤ 2 / overdue), and sort (newest joined / soonest renewal / fewest classes remaining) — replace what used to be single-select pills, since pills don't compose and stop being usable once the student list runs into the hundreds (`src/components/student-filter-fields.tsx`). Picking a renewal filter narrows each row down to just the subscription(s) that matched it, so a student with one healthy and one overdue instrument doesn't bury the reason they're in the list. With no status in the URL at all (a fresh visit), the list defaults to Active only — pick "All statuses (incl. inactive)" from the status filter to un-filter and see everyone, including inactive students.
- **Tracking a student to call back later** (`StudentFollowUp`): for a student whose subscription lapsed or is ending but who isn't renewing right now (out of town, thinking it over) — the "Follow-ups" card on a student's page logs a note and a call-back date instead of relying on memory. Pending ones (not yet marked "Done") surface on the admin dashboard under "Student callbacks due", soonest first, with overdue ones (past their date and still not done) called out rather than silently dropped off the list. Mirrors the same note/date/done shape leads already use (`FollowUp`), kept as its own model rather than a shared/polymorphic table since leads and students are otherwise unrelated records.
- **Moving a student to a different instrument** (e.g. Drums to Keyboard): the "Edit" on any enrollment now offers batches across every instrument, not just other day/times of the current one — picking one just retargets that `Enrollment` row's `batchId`. Safe to do freely: a `Subscription` pools by (student, course) from `Attendance` directly, never from the `Enrollment` row, so this never touches any subscription's usage. The old instrument's subscription (if any) is left exactly as it was; the new instrument shows its own "no active subscription" banner same as any fresh enrollment, prompting you to add one when the student's ready to start a package there.

## Billing

- **Raising an invoice**: "Create invoice" (`/admin/billing/new`) disables itself the moment it's clicked and stays disabled until the request finishes, so a slow connection or an impatient double-click can't raise the same invoice twice (`src/components/submit-button.tsx`).
- **Deleting an invoice raised by mistake**: "Delete invoice" appears on an invoice's page for as long as no payment has been recorded against it — a wrong student picked, a duplicate from a double-click. Once a payment exists, the invoice is real history and the option disappears; "Cancel invoice" (which keeps the row and just marks it void) is the correct way to void it from that point on.
