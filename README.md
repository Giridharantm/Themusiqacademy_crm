...
- **Correcting a mistake on the active subscription**: "Edit subscription" (next to "Renew / change subscription") updates the still-active `Subscription` row directly — plan, base classes, start date, expiry date, carry-forward — with no side effects on usage stats and no new subscription row created. Use this for typos and corrections (wrong start date entered, wrong plan picked); use "Renew" instead when the student is actually starting a new package.
